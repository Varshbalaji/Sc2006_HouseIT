...
from .models import User, Mastertowndata, Masterflattypes, Masterstreetdata, Masterflatmodeldata, Masterpropertyrentaldata, Masterpropertyresaledata,Listedproperty, Userbookmarks
...
from .getBookmarkedProperties import getBookmarkedProperties
# Shared Views
...
@login_required
def bookmarkedproperties(request):
    streetlist = Masterstreetdata.objects.all().order_by('streetname')
    townlist = Mastertowndata.objects.all().order_by('townname')
    flattypelist = Masterflattypes.objects.all().order_by('flattype')
    ...
    if request.method == 'GET':
        ...
        uritownname = uritownname.replace('+', ' ')
        uristreetname = uristreetname.replace('+', ' ')
        properties = getBookmarkedProperties(uritownname,uristreetname,uriflattype,uribeds,uribaths,urigarage,loggeduser)

    return render(request, 'houseitcommon/bookmarkedproperties.html',
                  {'streetlist': streetlist, 'townlist': townlist, 'flattypelist': flattypelist, 'properties': properties,
                   'uritownname':uritownname,'uristreetname': uristreetname, 'uriflattype': uriflattype, 'uribeds': uribeds
                   , 'uribaths': uribaths, 'urigarage': urigarage, 'loggeduser': loggeduser})
...
def showPropertyDetail(request,pk) :
    ...
    listURL = request.build_absolute_uri()
    bookmark=''
    if Userbookmarks.objects.filter(userid=userid, propertyid=pk).exists():
        bookmark = 'Y'
    else:
        bookmark = 'N'

    if 'bm' in request.GET:
        bookmarkflag = request.GET.get('bm')
        if bookmarkflag == 'ba':
            if bookmark == 'N':
                try:
                    a = Userbookmarks(userid=userid, propertyid=pk)
                    a.save()
                    bookmark = 'Y'
                except:
            ...
            if bookmark == 'Y':
                try:
                    a = Userbookmarks.objects.filter(userid=userid, propertyid=pk).delete()
                    #a.save()
                    bookmark = 'N'
                except:
                    print("Bookmark delete failed")

    # print(userid,'**',pk,'**',bookmark)
